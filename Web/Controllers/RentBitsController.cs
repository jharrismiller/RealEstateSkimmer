    ...
    public class RentBitsController : Controller
    {
        ...
        public async Task<IActionResult> GetDataForCurrentDbZips()
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                foreach (var needDataParameters in needingDataByZip)
                {
                ...
                var needingDataByCity = (
                 from p in _context.Property
                 let rb = _context.RentBits.FirstOrDefault(r => r.Location == (p.City + "," + p.State) && r.Type == "House")
                 where rb == null && p.City != null
                 select new { Location = (p.City + "," + p.State), PropertyType = "H" }
                 ).Union(
                  from p in _context.Property
                  let rb = _context.RentBits.FirstOrDefault(r => r.Zip == (p.City + "," + p.State) && r.Type == "Apartment")
                  where rb == null && p.City != null
                  select new { Location = (p.City + "," + p.State), PropertyType = "A" }
                 ).Distinct().ToList();

                foreach (var needDataParameters in needingDataByCity)
                {
                    var url = string.Format(baseStringUrl, needDataParameters.Location, needDataParameters.PropertyType);
                    using (HttpResponseMessage response = await client.GetAsync(url))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            using (HttpContent content = response.Content)
                            {
                                ...
                                if (td.Length == 5)
                                {
                                    var rent = new RentBits()
                                    {
                                        Location = needDataParameters.Location,
                                        Url = url,
                                        OneBedRoom = NumberHelper.GetNumber(td[2].TextContent) ?? 0,
                                        TwoBedRoom = NumberHelper.GetNumber(td[3].TextContent) ?? 0,
                                        ThreeOrMoreBedRoom = NumberHelper.GetNumber(td[4].TextContent) ?? 0,
